...
/* La place du gardien i dans le labyrinthe devient EMPTY */
void Labyrinthe::remove_guard_data(int i) {
    _data[(int)(_guards[i] -> _x/scale)][(int) (_guards[i] -> _y/scale)] = EMPTY;
}
...
/* La case du labyrinthe des coordonnees x et y devient EMPTY */
void Labyrinthe::remove_data(int x, int y) {
    _data[(int)(x/scale)][(int)(y/scale)] = EMPTY;
}
...
void Labyrinthe::calcul_distances_tresor() {
    ...
    while(!q.empty()) {
        Pos p = q.front();
        q.pop();
        if(p.x > 0 && p.x < LAB_WIDTH && p.y > 0 && p.y < LAB_HEIGHT && !isWall(p.x,p.y) && _tresor_distances[p.x][p.y] == -1 || _tresor_distances[p.x][p.y] > p.dist) {
...
Labyrinthe::Labyrinthe (char* filename) {
    ...
    // Ci-dessous initialisation de _data et _tresor_distances (ne marche pas)
    /*_data = new int*[width()];
    _tresor_distances = new int*[width()];
    for(int i = 0; i < width(); i++) {
        _data[i] = new int[height()];
        _tresor_distances[i] = new int[height()];
        }*/
    int * info;
